...
$$
w_{ij}=\frac{\left | N(u)\bigcap N(v) \right |}{\left | N(u)\bigcup N(v) \right |}
$$

（2）余弦相似度：

$$
w_{ij}=\frac{\left | N(u)\bigcap N(v) \right |}{\sqrt{\left | N(u) \right |\left |  N(v) \right |}}
$$

（3）改进后的余弦相似度：两个用户都买过某个热门物品，并不能代表他们兴趣相似，因此需要惩罚热门物品对相似度的影响

$$
w_{ij}=\frac{\sum_{i \in N(u)\bigcap N(v)}\frac{1}{log(1+\left | N(i)  \right |)}}{\sqrt{\left | N(u) \right |\left |  N(v) \right |}}
$$
...
#### 计算用户对物品的兴趣度
在得到物品之间的相似度后，将和用户喜欢的每个物品x最相似的前K个物品推荐给用户，其中用户u对物品i的兴趣度可以通过下式计算：

$$
p(u,i)=\sum_{j \in S(x,K),x \in N(u)}w_{ji}r_{ui}
$$

- $S(x,K)$:和物品x最相似的K个物品集合
