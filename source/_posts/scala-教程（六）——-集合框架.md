...
> 集合的常见操作：创建、增删改查、拆分合并、测试、转换、迭代、mapreduce
...
![-c](https://likeitea-1257692904.cos.ap-guangzhou.myqcloud.com/liketea_blog/20191203214915.png)

其中虚线代表隐式转换、粗实线代表默认实现、细实线代表实现：

![-c](https://likeitea-1257692904.cos.ap-guangzhou.myqcloud.com/liketea_blog/20191203215018.png)
...
- `scala.collection.generic`包包含了集合的构建块，集合类延迟了`collection.generic` 类中的部分操作实现；
...
可遍历（Traversable） 是容器（collection）类的最高级别特质，它唯一的抽样操作是`foreach`，`foreach` 是 `Traversable` 所有操作的基础。foreach 方法用于遍历容器中所有元素，并对每个元素进行指定的操作，Elem 是容器中元素的类型，U是一个任意的返回值类型，对f的调用仅仅是容器遍历的副作用，实际上所有计算结果都被foreach跑起来（没有返回值）：
...
可迭代是容器类的另一个特质，这个特质里所有方法的定义都基于一个抽象方法`iterator`，从`Traversable Trait`中继承来的foreach方法在这里也是利用iterator来实现的：
...
Seq trait 具有三个子特征（subtrait）：线性序列`LinearSeq` 和 索引序列`IndexedSeq`不添加任何新的操作，但提供不同的性能特点；缓冲器 `Buffer` 是可变序列的一个重要种类
...
| Queue |  | ✓ |  <div>A first-in, first-out data structure.</div>|
| Range | ✓ |  |  <div>A range of integer values.</div> |
| Stack |  | ✓ |  <div>A last-in, first-out data structure.</div> |
| Stream |  | ✓ |  <div>Similar to List, but it’s lazy and persistent. Good for a large or infinite sequence, similar to a Haskell List.</div> |
| String | ✓ |  |  <div>Can be treated as an immutable, indexed sequence of characters.</div>|
| Vector | ✓ |  |  <div>The “go to” immutable, indexed sequence. The Scaladoc describes it as, “Implemented as a set of nested arrays that’s efficient at splitting and joining.”</div>|
